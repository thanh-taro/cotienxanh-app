...
class LevelHardestButton extends Phaser.GameObjects.Sprite {
  static get KEY () {
    return 'LevelHardestButton'
  }

  static preload (scene) {
    const { asset, assetWidth, assetHeight } = loadAsset(scene, assetSpec)
    scene.load.spritesheet(LevelHardestButton.KEY, asset, { frameWidth: assetWidth, frameHeight: assetHeight })
  }

  constructor (scene, addToScene = true, config = {}) {
    const { scale } = loadAsset(scene, assetSpec)

    const x = scene.cameras.main.width / 5 * 4
    const y = scene.cameras.main.height / 2

    super(scene, x, y, LevelHardestButton.KEY, 0)
...
export default LevelHardestButton
