...
class LevelHardButton extends Phaser.GameObjects.Sprite {
  static get KEY () {
  ...
  static preload (scene) {
    ...
    scene.load.spritesheet(LevelHardButton.KEY, asset, { frameWidth: assetWidth, frameHeight: assetHeight })
  }

  constructor (scene, addToScene = true, config = {}) {
    const { scale } = loadAsset(scene, assetSpec)

    const x = scene.cameras.main.width / 5 * 3
    const y = scene.cameras.main.height / 2

    super(scene, x, y, LevelHardButton.KEY, 0)
...
export default LevelHardButton
