...
class MoveTheImagesToTheRightPositionsScene extends Phaser.Scene {
  ...
  addAnswers (answersData) {
    ...
    var total = Object.keys(answersData).length < 3 ? 3 : Object.keys(answersData).length
    var number = 0;
    for (let key in answersData) {
      ...
      if ((this.things.level === 'hardest' && answerKey === questionCard.key) || (this.things.level !== 'hardest' && answersData[key])) {
  ...
  configTheQuestionCard (number, total) {
    ...
    const startX = this.cameras.main.width * 5 / 100
    const endX = this.cameras.main.width / 7 * 5
    let width = (endX - startX - padding * (total - 1)) / total
    const height = this.cameras.main.height * 80 / 100
  ...
  configTheAnswerCard (number, total) {
    ...
    const startX = (this.cameras.main.width * 95 / 100) / 7 * 5 + (this.cameras.main.width * 5 / 100)
    const startY = this.cameras.main.height * 10 / 100
    const endX = this.cameras.main.width * 95 / 100
  ...
  createSpeaker () {
    let x = this.cameras.main.width * 95 / 100
    let y = this.cameras.main.height * 90 / 100
    this.things.speaker = new Cards(this, 'speakerI', 0, { x: x, y: y, scale: 0.4, hasSound: false, allowClick: true }, null, true, {}, () => this.things.questionSound.play())
  }
